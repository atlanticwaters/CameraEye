...
struct TypographyDemoView: View {
            ...
            VStack(alignment: .leading, spacing: DesignSystemGlobal.Spacing4) {
                ...
                VStack(alignment: .leading, spacing: DesignSystemGlobal.Spacing2) {
                    ...
                    Text("Experience professional grilling results with 39,000 BTUs of cooking power across three stainless steel burners. The porcelain-enameled lid retains heat efficiently while the Flavorizer bars add that signature smoky taste.")
                        .thdFont(.bodyMd)
                        .foregroundColor(DesignSystemGlobal.TextOnContainerColorPrimary)
                        .lineSpacing(DesignSystemGlobal.FontSizeBodyMd * (DesignSystemGlobal.FontLineHeightBase - 1))
    ...
    private var productCardExample: some View {
        VStack(alignment: .leading, spacing: DesignSystemGlobal.Spacing2) {
            ...
            DSBadge(label: "Sale", size: .small, variant: .filledStrong, color: .danger)
        ...
        HStack(alignment: .top, spacing: DesignSystemGlobal.Spacing2) {
            ...
            VStack(alignment: .leading, spacing: DesignSystemGlobal.Spacing1) {
                ...
                Text("Your order #12345 has been placed successfully. You'll receive a confirmation email shortly.")
                    .thdFont(.bodySm)
                    .foregroundColor(DesignSystemGlobal.TextOnContainerColorSecondary)
                    .lineSpacing(DesignSystemGlobal.FontSizeBodySm * (DesignSystemGlobal.FontLineHeightBase - 1))
            }
    ...
    private func fontStatusRow(for fontName: String) -> some View {
        ...
        return HStack {
            Image(systemName: isLoaded ? "checkmark.circle.fill" : "xmark.circle.fill")
                .foregroundColor(isLoaded ? DesignSystemGlobal.IconOnContainerColorSuccess : DesignSystemGlobal.IconOnContainerColorError)
            ...
            Text(isLoaded ? "Loaded" : "Missing")
                .thdFont(.caption)
                .foregroundColor(isLoaded ? DesignSystemGlobal.TextOnSurfaceColorSuccess : DesignSystemGlobal.TextOnSurfaceColorError)
        }
    }
